...
class AccountGeneralLedgerReportHandler(models.AbstractModel):
    ...
    def _overwrite_group_value(self, values, new_value, group, currency_id):
        # Si la línea ya estaba en una moneda del grupo, mantenemos esa moneda
        # (aunque el valor no haya cambiado); si no, mostramos la de referencia.
        _logger.info("Overwrite group!")
        display_currency_id = (
            currency_id if currency_id in group.currency_ids.ids else group.reference_currency_id.id
        )
        values["balance"] = new_value
        values["amount_currency"] = new_value
        values["currency_id"] = display_currency_id
